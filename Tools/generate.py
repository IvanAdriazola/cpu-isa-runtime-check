...
def normalize(line: str) -> str:
    """
    Normaliza una linea de assembly para poder comparar lo que el
    usuario escribio contra lo que ndisasm devuelve, ignorando
    diferencias cosmeticas (espacios, mayusculas/minusculas, comas
    pegadas o separadas).
    """
...
def extract_written_mnemonics(source: str) -> list[str]:
    """
    Extrae las instrucciones tal como el usuario las escribio en TESTS,
    ignorando lineas vacias, comentarios y etiquetas.
    """
...
for name, source in TESTS.items():
    ...
    try:
        ...
        # -O0 desactiva el optimizador de NASM: evita que elija
        # silenciosamente una codificacion alternativa "mas inteligente"
        # a la que escribiste explicitamente.
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # Verificacion 1: roundtrip binario
        # Bytes originales == bytes regenerados tras bin -> asm -> bin
        # -------------------------------------------------------------

        binary_roundtrip_ok = original == regenerated

        if not binary_roundtrip_ok:
            raise RuntimeError(
        ...
        # -------------------------------------------------------------
        # Verificacion 2: roundtrip de mnemonic
        # Lo que el usuario escribio en TESTS == lo que ndisasm
        # interpreto que es esa secuencia de bytes.
        # Esto detecta el caso donde NASM elige una forma equivalente
        # pero distinta (p.ej. VEX en vez de legacy) que igual pasaria
        # el roundtrip binario porque el ciclo completo es autoconsistente,
        # pero que no es la instruccion que realmente querias emitir.
        # -------------------------------------------------------------

        written = extract_written_mnemonics(source)
        disassembled = [normalize(inst) for inst in assembly]

        mnemonic_roundtrip_ok = written == disassembled

        if not mnemonic_roundtrip_ok:
            raise RuntimeError(
                "Mnemonic roundtrip mismatch (NASM pudo haber elegido "
                "una codificacion distinta a la escrita):\n"
                f"  escrito       = {written}\n"
        ...
        # -------------------------------------------------------------
        # Emit Python: la info va en un docstring dentro de la funcion,
        # asi que todo el bloque es codigo valido para copiar/pegar tal cual.
        # -------------------------------------------------------------
